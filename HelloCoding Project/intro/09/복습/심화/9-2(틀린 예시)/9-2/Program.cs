...
namespace _9_2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] S_Scores = new int[S_Numbers * C_Numbers];
            ...
            for (int C_Count = 0; C_Count < C_Numbers; C_Count++)
            {
                ...
                {
                    int S_Count = 0;
                    for (S_Count = S_Count + (S_Numbers * C_Count); S_Count < S_Numbers * (C_Count + 1); S_Count++)
                    {
                        Console.Write(C_Count);
                        Console.Write("반 ");
                        Console.Write(S_Count - (C_Count * S_Numbers));
                        Console.WriteLine("번째 학생의 성적을 입력해주세요.");
